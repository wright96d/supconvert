...
If no mode or other flags are specified, supconvert defaults to the HDR paper white standard of 58% (~200nits). All brightness mapping modes display nits measurement under 68% (~500nits). (--nits overrides threshold) The HDR pipeline follows the SDR to HDR conversion formula outlined in BT.2408 §5.1.2.
